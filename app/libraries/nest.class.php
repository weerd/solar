...
class Nest {
    ...
    function __construct($username=null, $password=null) {
        ...
        $this->cookie_file = app_path() . '/storage/cache/nest_php_cookies_' . md5($username . $password);
        static::secure_touch($this->cookie_file);

        $this->cache_file = app_path() . '/storage/cache/nest_php_cache_' . md5($username . $password);
    ...
    private function doRequest($method, $url, $data_fields=null, $with_retry=TRUE) {
        ...
        if (DEBUG) {
            ...
            $curl_cainfo = app_path() . '/storage/cache/' . 'cacert.pem';
    ...
    private static function secure_touch($fname) {
        ...
        $temp = tempnam(app_path() . '/storage/cache', 'NEST');
        rename($temp, $fname);
    }
}
